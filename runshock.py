...
vshock=6.5e5
tgas = 300.
solutions, Jrad, vshock = main.shock_main(sizex=3e10, numpoints=1e4, nspecs=ngas,
    ndust=ndust, v0=vshock, niter=0, ncpu=2, t0=tgas)
#
# Write out the solutions
#
fileio.writeOut(solutions=solutions, Jrad=Jrad,
    vshock=vshock, fname='shockout')
#
# Read the output
#
...
#
x0 = sols.attrs['x']/sols.attrs['vshock']
x0 = x0/3600.0
#
# Plot these
#
...
ax0.plot(x0, sols.attrs['vgas']*1e-5, 'b-', lw=1.2, alpha=0.6,
    label=r'\boldmath$\upsilon_{\rm gas}$')
if ndust is not None:
    ax0.plot(x0, sols.attrs['vdust']*1e-5, 'b-.', lw=1.2,
...
ax1 = ax0.twinx()
ax1.plot(x0, sols.attrs['tgas'], 'g-', lw=1.2, alpha=0.6,
    label=r'\boldmath$T_{\rm gas}$')
if ndust is not None:
    ax1.plot(x0, sols.attrs['tdust'], 'r-.', lw=1.2, alpha=0.6,
...
ax0.semilogy(x0, sols[:,0], 'k-', lw=1.2,
    label=r'\boldmath${\rm H}$')
ax0.semilogy(x0, sols[:,1], 'b-', lw=1.2,
    label=r'\boldmath${\rm H_{2}}$')
ax0.semilogy(x0, sols[:,2], 'g-', lw=1.2,
    label=r'\boldmath${\rm He}$')
if ndust is not None:
    ax0.semilogy(x0, sols.attrs['ndust']*1e15, 'k--', lw=1.2,
...
print sols.attrs['tau']
print sols.attrs['Jrad'][1]
ax0.semilogy(x0, sols.attrs['tau'], 'b-', lw=1.2)
ax0 = fig_labs(ax=ax0, xlab=r'\textbf{\boldmath$t$ [h]}',
...
ax1 = ax0.twinx()
ax1.plot(x0, np.log10(sols.attrs['Jrad'][1]), 'go', lw=1.2, ms=5)
ax1 = fig_labs(ax=ax1, xlab=r'\textbf{\boldmath$t$ [h]}',
